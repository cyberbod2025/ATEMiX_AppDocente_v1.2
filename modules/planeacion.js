...
HEAD
  ...
  icalBtn?.addEventListener('click',()=>{ const d=new Date(startInput.value||new Date()); const mon=mondayOf(d); const p=loadPlan(); const k=weekKey(mon); const w=p.planner?.[k]; if(!w){ alert('No hay eventos en esta semana.'); return;} const pad=(n)=> String(n).padStart(2,'0'); const fmtDate=(dt)=> `${dt.getFullYear()}${pad(dt.getMonth()+1)}${pad(dt.getDate())}`; const fmtDT=(dt)=> `${fmtDate(dt)}T${pad(dt.getHours())}${pad(dt.getMinutes())}00`; const toDate=(dayIdx,timeStr)=>{ const base=addDays(mon,dayIdx); const [hh,mm]=(timeStr||'07:30').split(':').map(x=>parseInt(x,10)||0); const dt=new Date(base); dt.setHours(hh,mm,0,0); return dt; }; let ics='BEGIN:VCALENDAR\nVERSION:2.0\nPRODID:-//AtemiMX//Planner//ES\n'; w.days.forEach((arr,di)=>{ (arr||[]).forEach((it,j)=>{ const dt=toDate(di,it.time||'07:30'); const uid=`${fmtDT(dt)}-${di}-${j}@atemix`; ics+='BEGIN:VEVENT\n'; ics+=`UID:${uid}\n`; ics+=`DTSTAMP:${fmtDT(new Date())}\n`; ics+=`DTSTART:${fmtDT(dt)}\n`; ics+=`SUMMARY:${(it.title||'Clase').replace(/\n/g,' ')}\n`; if(it.link) ics+=`URL:${it.link}\n`; ics+='END:VEVENT\n'; }); }); ics+='END:VCALENDAR\n'; const blob=new Blob([ics],{type:'text/calendar'}); const a=document.createElement('a'); a.href=URL.createObjectURL(blob); a.download=`planner_${fmtDate(mon)}.ics`; a.click(); });
  //  d5abf1aee67ca666132b52c9aa738c2288b9ca45
